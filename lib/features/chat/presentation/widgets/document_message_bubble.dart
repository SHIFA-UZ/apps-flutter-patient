...
class DocumentMessageBubble extends StatelessWidget {
  ...
  Future<void> _openDocument(BuildContext context, String url) async {
    try {
      if (await canLaunchUrlString(url)) {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(AppLocalizations.of(context)!.translate('cannotOpenDocumentUrl') ?? 'Cannot open document'),
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${AppLocalizations.of(context)!.translate('errorOpeningDocument')}: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    final fileName = message.content.fileName ?? (l10n.translate('document') ?? 'Document');
