import 'package:country_code_picker/country_code_picker.dart';
import 'package:flutter/material.dart';

/// A form field that combines a country code dropdown with a phone number
/// text field. Use for Firebase/SMS-ready E.164 format (e.g. +998901234567).
class PhoneInputField extends StatefulWidget {
  const PhoneInputField({
    super.key,
    this.initialFullPhone,
    this.labelText,
    this.hintText,
    this.validator,
    this.onChanged,
    this.enabled = true,
    this.autofocus = false,
  });

  /// Full phone in E.164 or similar (e.g. +998901234567). Used to set initial country and number.
  final String? initialFullPhone;
  final String? labelText;
  final String? hintText;
  final String? Function(String? value)? validator;
  final void Function(String fullPhone)? onChanged;
  final bool enabled;
  final bool autofocus;

  @override
  State<PhoneInputField> createState() => _PhoneInputFieldState();
}

class _PhoneInputFieldState extends State<PhoneInputField> {
  late CountryCode _countryCode;
  late TextEditingController _numberController;

  /// Parses [fullPhone] (e.g. +998901234567) into dial code and national number.
  static ({String dialCode, String number}) parseFullPhone(String? fullPhone) {
    if (fullPhone == null || fullPhone.isEmpty) {
      return (dialCode: '+998', number: '');
    }
    final digits = fullPhone.replaceAll(RegExp(r'\D'), '');
    if (digits.isEmpty) return (dialCode: '+998', number: '');
    // Heuristic: try common dial code lengths (1–4 digits)
    for (final len in [4, 3, 2, 1]) {
      if (digits.length >= len) {
        final code = digits.substring(0, len);
        final rest = digits.substring(len);
        if (code.startsWith(RegExp(r'[1-9]'))) {
          return (dialCode: '+$code', number: rest);
        }
      }
    }
    return (dialCode: '+998', number: fullPhone.replaceAll(RegExp(r'\D'), ''));
  }

  @override
  void initState() {
    super.initState();
    final parsed = parseFullPhone(widget.initialFullPhone);
    _countryCode = CountryCode(
      dialCode: parsed.dialCode,
      code: _dialCodeToCountryCode(parsed.dialCode),
      name: '',
    );
    _numberController = TextEditingController(text: parsed.number);
    _numberController.addListener(_notifyChanged);
  }

  String _dialCodeToCountryCode(String dialCode) {
    const map = {
      '+1': 'US', '+7': 'RU', '+20': 'EG', '+27': 'ZA', '+30': 'GR',
      '+31': 'NL', '+32': 'BE', '+33': 'FR', '+34': 'ES', '+36': 'HU',
      '+39': 'IT', '+41': 'CH', '+43': 'AT', '+44': 'GB', '+45': 'DK',
      '+46': 'SE', '+47': 'NO', '+48': 'PL', '+49': 'DE', '+51': 'PE',
      '+52': 'MX', '+53': 'CU', '+54': 'AR', '+55': 'BR', '+56': 'CL',
      '+57': 'CO', '+58': 'VE', '+60': 'MY', '+61': 'AU', '+62': 'ID',
      '+63': 'PH', '+64': 'NZ', '+65': 'SG', '+66': 'TH', '+81': 'JP',
      '+82': 'KR', '+84': 'VN', '+86': 'CN', '+90': 'TR', '+91': 'IN',
      '+92': 'PK', '+93': 'AF', '+94': 'LK', '+95': 'MM', '+98': 'IR',
      '+212': 'MA', '+213': 'DZ', '+216': 'TN', '+218': 'LY', '+220': 'GM',
      '+221': 'SN', '+222': 'MR', '+223': 'ML', '+224': 'GN', '+225': 'CI',
      '+226': 'BF', '+227': 'NE', '+228': 'TG', '+229': 'BJ', '+230': 'MU',
      '+231': 'LR', '+232': 'SL', '+233': 'GH', '+234': 'NG', '+235': 'TD',
      '+236': 'CF', '+237': 'CM', '+238': 'CV', '+239': 'ST', '+240': 'GQ',
      '+241': 'GA', '+242': 'CG', '+243': 'CD', '+244': 'AO', '+245': 'GW',
      '+246': 'IO', '+248': 'SC', '+249': 'SD', '+250': 'RW', '+251': 'ET',
      '+252': 'SO', '+253': 'DJ', '+254': 'KE', '+255': 'TZ', '+256': 'UG',
      '+257': 'BI', '+258': 'MZ', '+260': 'ZM', '+261': 'MG', '+262': 'RE',
      '+263': 'ZW', '+264': 'NA', '+265': 'MW', '+266': 'LS', '+267': 'BW',
      '+268': 'SZ', '+269': 'KM', '+290': 'SH', '+291': 'ER', '+297': 'AW',
      '+298': 'FO', '+299': 'GL', '+350': 'GI', '+351': 'PT', '+352': 'LU',
      '+353': 'IE', '+354': 'IS', '+355': 'AL', '+356': 'MT', '+357': 'CY',
      '+358': 'FI', '+359': 'BG', '+370': 'LT', '+371': 'LV', '+372': 'EE',
      '+373': 'MD', '+374': 'AM', '+375': 'BY', '+376': 'AD', '+377': 'MC',
      '+378': 'SM', '+379': 'VA', '+380': 'UA', '+381': 'RS', '+382': 'ME',
      '+383': 'XK', '+385': 'HR', '+386': 'SI', '+387': 'BA', '+389': 'MK',
      '+420': 'CZ', '+421': 'SK', '+423': 'LI', '+500': 'FK', '+501': 'BZ',
      '+502': 'GT', '+503': 'SV', '+504': 'HN', '+505': 'NI', '+506': 'CR',
      '+507': 'PA', '+508': 'PM', '+509': 'HT', '+590': 'GP', '+591': 'BO',
      '+592': 'GY', '+593': 'EC', '+594': 'GF', '+595': 'PY', '+596': 'MQ',
      '+597': 'SR', '+598': 'UY', '+599': 'CW', '+670': 'TL', '+672': 'NF',
      '+673': 'BN', '+674': 'NR', '+675': 'PG', '+676': 'TO', '+677': 'SB',
      '+678': 'VU', '+679': 'FJ', '+680': 'PW', '+681': 'WF', '+682': 'CK',
      '+683': 'NU', '+685': 'WS', '+686': 'KI', '+687': 'NC', '+688': 'TV',
      '+689': 'PF', '+690': 'TK', '+691': 'FM', '+692': 'MH', '+850': 'KP',
      '+852': 'HK', '+853': 'MO', '+855': 'KH', '+856': 'LA', '+880': 'BD',
      '+886': 'TW', '+960': 'MV', '+961': 'LB', '+962': 'JO', '+963': 'SY',
      '+964': 'IQ', '+965': 'KW', '+966': 'SA', '+967': 'YE', '+968': 'OM',
      '+970': 'PS', '+971': 'AE', '+972': 'IL', '+973': 'BH', '+974': 'QA',
      '+975': 'BT', '+976': 'MN', '+977': 'NP', '+992': 'TJ', '+993': 'TM',
      '+994': 'AZ', '+995': 'GE', '+996': 'KG', '+998': 'UZ',
    };
    return map[dialCode] ?? 'UZ';
  }

  String get _fullPhone {
    final digits = _numberController.text.replaceAll(RegExp(r'\D'), '');
    final dial = _countryCode.dialCode ?? '+998';
    return '$dial$digits';
  }

  void _notifyChanged() {
    widget.onChanged?.call(_fullPhone);
  }

  @override
  void didUpdateWidget(PhoneInputField oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.initialFullPhone != widget.initialFullPhone) {
      final parsed = parseFullPhone(widget.initialFullPhone);
      _countryCode = CountryCode(
        dialCode: parsed.dialCode,
        code: _dialCodeToCountryCode(parsed.dialCode),
        name: '',
      );
      _numberController
        ..removeListener(_notifyChanged)
        ..text = parsed.number;
      _numberController.addListener(_notifyChanged);
    }
  }

  @override
  void dispose() {
    _numberController.removeListener(_notifyChanged);
    _numberController.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        CountryCodePicker(
          onChanged: (CountryCode code) {
            setState(() => _countryCode = code);
            _notifyChanged();
          },
          initialSelection: _countryCode.code,
          showCountryOnly: false,
          showOnlyCountryWhenClosed: false,
          alignLeft: false,
          padding: EdgeInsets.zero,
          showFlag: true,
          dialogSize: Size(MediaQuery.of(context).size.width * 0.9, MediaQuery.of(context).size.height * 0.7),
        ),
        const SizedBox(width: 8),
        Expanded(
          child: TextFormField(
            controller: _numberController,
            keyboardType: TextInputType.phone,
            enabled: widget.enabled,
            autofocus: widget.autofocus,
            decoration: InputDecoration(
              labelText: widget.labelText,
              hintText: widget.hintText ?? '901234567',
              prefixIcon: const Icon(Icons.phone),
            ),
            validator: widget.validator != null
                ? (value) {
                    final full = _fullPhone;
                    final digitsOnly = full.replaceAll(RegExp(r'\D'), '');
                    final dialCodeOnly = (_countryCode.dialCode ?? '').replaceAll(RegExp(r'\D'), '');
                    final nationalLength = digitsOnly.length - dialCodeOnly.length;
                    final isEmpty = nationalLength <= 0;
                    return widget.validator!(isEmpty ? null : full);
                  }
                : null,
          ),
        ),
      ],
    );
  }
}

/// Helper to get the full E.164 phone from a [PhoneInputField] state.
/// Use with a [GlobalKey<PhoneInputFieldState>] or pass value from onChanged.
String normalizePhoneForSms(String? fullPhone) {
  if (fullPhone == null || fullPhone.isEmpty) return '';
  final digits = fullPhone.replaceAll(RegExp(r'\D'), '');
  if (digits.isEmpty) return '';
  return fullPhone.trim().startsWith('+') ? fullPhone.trim() : '+$digits';
}
